...
#'
#' For a detailed explanation, please consult the vignette of the csarGeo package.
#'
...
#'
#' @importFrom terra crs crop mask freq res vect
#' @importFrom sf st_as_sf st_crs st_transform st_buffer st_geometry st_intersection st_area st_intersects st_union st_convex_hull st_geometry_type st_coordinates st_drop_geometry
#'
#' @export
#'
#' @examples
...
countryside_sar <- function(
  ...
  #---------------------------- 3. Helper functions ----------------------------
  # ── Method-specific helpers (defined only for the relevant method) ──────────
  ...
  if (method == "circles") {
    ...
    extract_species_positions <- function(species_habitat_matrix, species_site_matrix) {
      ...
      location_ids <- species_site_matrix[, 1]
      species_data <- species_site_matrix[, -1]
      habitat_positions <- list()

      for (habitat in habitat_names) {
        species_in_habitat <-
          rownames(species_habitat_matrix)[species_habitat_matrix[, habitat] == 1]
        species_positions <- which(colnames(species_data) %in% species_in_habitat)
        habitat_positions[[habitat]] <- species_positions
      }
      attr(habitat_positions, "location_ids") <- location_ids
      return(habitat_positions)
    }
  }

  # ── Shared helpers (always defined, regardless of method) ───────────────────
  ...
  if (method == "circles") {
    ...
    if (n_runs > 1) {
      ...
      avg_sar_results <- if (length(runs) > 0 && runs[[1]]$sar_analysis$valid) {
        runs[[1]]$sar_analysis
      } else {
        list(valid = FALSE, message = "SAR analysis not valid")
      }
    }

    # ----- Step 5: Return results for circles -----
    res <- list(
      method          = method,
      n_runs          = n_runs,
      runs            = runs,
      avg_sar_results = avg_sar_results,   # <-- averages live here
      points_sf       = points_sf,
