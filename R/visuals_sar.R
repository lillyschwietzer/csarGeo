...
#' @param plot_level_n Plot only the n-th clusterung level as a solo plot. Defaults to NULL.
#'
  ...
  if (method == "circles") {
    ...
    plot_heatmap <- function(csar_results) {
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
        stop("Package 'ggplot2' is required for heatmap plotting. Please install it.")
      }
      if (!requireNamespace("reshape2", quietly = TRUE)) {
        stop("Package 'reshape2' is required for heatmap plotting. Please install it.")
      }
      ...
      ggplot2::ggplot(affinity_long,
        ...
        ggplot2::scale_fill_gradientn(
          colors = c("white", "lightblue", "steelblue", "darkblue"),
          name = "Affinity",
          limits = c(0, 1)
        ) +
