...
Both of "circles" and "clusters" are explained in detail under 2.2 and 2.3. A high-resolution land-use raster further increases the accuracy of the pixel-based calculation of the habitat areas and minimizes edge effects around the polygons.
...
The analysis function `countryside_sar()` needs three distinct input files. An example of each is provided as default data of the csarGeo package, which can be used to run an example analysis (s. 5).

The first file, the **species data table**, needs a location ID as the first column, the longitude and latitude values as its second and third columns and binary presence-absence species data from column 4 and onward. The function receives this table as the parameter "data".
